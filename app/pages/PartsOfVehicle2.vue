    ...
    <p>Categoria:</p>
                       <AutoCompletame v-model="catTextFilter" :state1="catTextFilter" :links="this.categorias"  @update:selectedValue="handleCatFilter"  valueKey="name" >
                          ...
                          <div class="row-sm-2 align-self-end position-relative">
                            ...
                            <base-button
                                          @click="resetFilteredCarPart();setFiltros('vehicleId','639e1753b4480b09ec4ddde1');setFiltros('type','');setFiltros('carMaker','');setFiltros('carModel','');catTextFilter='';makerTextFilter='';defaultPagination=1; filterTableModels();modelTextFilter='';"
                                          type="danger"
                                          class="col-sm-2"
                                          size="sm"
...
// // console.log("sort by: ");
    ...
    async fetch() {
        ...
        this.filterby.vehicleId=this.$route.query.vehicle._id;
        ...
        await this.$store.dispatch('getCarMakers');
         ...
         this.carMakers=JSON.parse(JSON.stringify(this.$store.state.carMakers)).sort(
              ...
              if(this.$route.query.vehicle._id!=undefined){
               await this.$store.commit('setSelectedVehicle',this.$route.query.vehicle);
...
generateRFID() {
   ...
   this.newCarPart.rfid = rfid;
      ...
      setFiltros(type,name){
        this.$store.commit('setFiltros',this.filtros); 
